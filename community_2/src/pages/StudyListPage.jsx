...
function StudyListPage() {
    ...
    useEffect(() => {
        const customStudies = JSON.parse(localStorage.getItem('customStudies')) || [];
        setStudies([...defaultStudies, ...customStudies]);
    }, []);
    ...
    const handleDelete = (id) => {
        const customStudies = JSON.parse(localStorage.getItem('customStudies')) || [];
        const updatedCustomStudies = customStudies.filter(study => study.id !== id);
        localStorage.setItem('customStudies', JSON.stringify(updatedCustomStudies));
        setStudies([...defaultStudies, ...updatedCustomStudies]);

        const filteredAfterDelete = selectedCategory === '전체'
            ? [...defaultStudies, ...updatedCustomStudies]
            : selectedCategory === '직접 작성'
                ? updatedCustomStudies
                : [...defaultStudies, ...updatedCustomStudies].filter(study => study.category === selectedCategory);
        const totalPagesAfterDelete = Math.ceil(filteredAfterDelete.length / studiesPerPage);
    ...
    const filteredStudies = selectedCategory === '전체'
        ? studies
        : selectedCategory === '직접 작성'
            ? studies.filter(study => study.category === '직접 작성')
...
const categories = ['전체', 'IT 개발', '언어', '디자인', '자격증', '자기개발', '직접 작성'];
